...
| Register and compare catalog provenance | `POST /catalog/source-artifacts`, `/versions`, `/import-runs`, or `/releases`, bounded `GET /catalog/releases?limit=&offset=`, or `GET /catalog/releases/{id}/compare/{id}` with `X-Purpose: catalog_governance` |
| Probe | `GET /healthz` |
