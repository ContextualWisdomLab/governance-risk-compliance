...
7. Read `/compliance-workspace` for one tenant-scoped posture projection combining controls, obligations, policy gaps, evidence-request workflow state, and versioned risk assessments with treatment and acceptance disposition.
...
| Create, assess, treat, and accept a risk | `POST /risk-methodologies`, `POST /risks`, `POST /risks/{id}/assessments`, `POST /risks/{id}/treatments`, `POST /risks/{id}/acceptances`, or `GET /risks`; disposition writes require an expected revision and independent acceptance actor |
...
| Read the tenant-scoped compliance workspace | `GET /compliance-workspace` with `compliance_governance`; returns distinct obligations, work-item queues, controls, policy gaps, evidence-request state, and risk assessments with disposition |
