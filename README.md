...
The HTTP surface is an **unauthenticated developer preview** unless `create_app(access_token_verifier=...)` is supplied. `X-Actor-Id` and `X-Purpose` declare audit context and purpose; they do not authenticate an actor. When a Keyverse verifier is configured, policy and evidence routes require a Bearer access token, use the verified subject as the actor, stamp the Keyverse `org` on owned rows, return only that officer's policies and tenant-owned policy gaps, and write audit events with issuer, client, and request correlation—never the raw token. One tenant's CSAP evidence cannot close another tenant's gap. The official reference catalog at `/controls` and `/healthz` stay public, while evidence-derived `/controls/uncovered` requires `grc.policy.read` and computes coverage only from the verified tenant's evidence. `/openapi.json` publishes that Keyverse Bearer contract. The command-line server binds to `127.0.0.1`, and the app always rejects proxy-forwarded or non-loopback traffic. Set `CWL_GRC_REQUIRE_KEYVERSE=1` with a reviewed JWKS file (`CWL_GRC_KEYVERSE_JWKS_PATH`) and readable TLS files to refuse header-identity HTTP starts; missing certificate or key files fail closed before Uvicorn starts. The same flag requires `CWL_GRC_ACCESS_TOKEN` for `cwl-grc policy author|revise`, `bind`, `policy list`, and `gaps`; `--actor` cannot impersonate the Keyverse subject. That is still a loopback preview. Persistent stores still need `CWL_GRC_EVIDENCE_KEY`. Follow `docs/runbooks/keyverse-deployment-hardening.md` for local-preview migration and issuer outage steps. Do not route external traffic until Keyverse-backed OIDC, tenant authorization, and production admission are implemented.
...
The data commands `policy author`, `policy revise`, `policy list`, `gaps`, and `bind` print JSON that states the next action. `cwl-grc serve` starts the local Uvicorn server and does not print data JSON. Running `cwl-grc policy` without `author`, `revise`, or `list` is invalid and exits with code 2. When `CWL_GRC_REQUIRE_KEYVERSE` is set, those data commands require `CWL_GRC_ACCESS_TOKEN` and return only the verified officer's tenant-owned rows. The CLI remains a loopback developer-preview interface until non-loopback admission exists.
...
| See evidence-derived catalog coverage gaps | `GET /controls/uncovered?framework=soc2_tsc_2017`; Keyverse mode requires `grc.policy.read` and returns the verified tenant's coverage only |
| Store evidence | `POST /evidence-records` with `X-Actor-Id` and `X-Purpose: evidence_binding` in local preview, or a Keyverse Bearer token in Keyverse mode |
