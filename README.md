...
7. Read `/compliance-workspace` for one tenant-scoped posture projection combining controls, obligations, policy gaps, evidence-request workflow state, versioned risk assessments with treatment/acceptance/closure disposition, and read-only risk portfolio indicators.
...
| Create, assess, treat, accept, and close a risk | `POST /risk-methodologies`, `POST /risks`, `POST /risks/{id}/assessments`, `POST /risks/{id}/treatments`, `POST /risks/{id}/acceptances`, `POST /risks/{id}/closures`, or `GET /risks`; disposition writes require an expected revision and independent actor |
...
| Read the tenant-scoped compliance workspace | `GET /compliance-workspace` with `compliance_governance`; returns distinct obligations, work-item queues, controls, policy gaps, evidence-request state, risk assessments with treatment/acceptance/closure disposition, and bounded risk portfolio indicators |
...
- Risk disposition is separate immutable history: versioned treatment plans set a due date and owner, while above-appetite acceptance is independently actor-approved, time-bounded, escalated above tolerance, and audit-recorded. Expiry remains a follow-up action; closed risks remain visible for audit but are excluded from overdue/actionable risk items, and portfolio indicators do not imply a composite risk score.
- Risk closure is separate immutable history: an independent actor can close only the latest within-appetite reassessment with a closure evidence reference. Closed risks retain all prior assessments and disposition records; closure is not a certification conclusion.
