...
7. Read `/compliance-workspace` for one tenant-scoped posture projection combining controls, obligations, policy gaps, evidence-request workflow state, and versioned risk assessments with treatment, acceptance, and closure disposition.
...
| Read the tenant-scoped compliance workspace | `GET /compliance-workspace` with `compliance_governance`; returns distinct obligations, work-item queues, controls, policy gaps, evidence-request state, and risk assessments with treatment, acceptance, and closure disposition |
