...
7. Read `/compliance-workspace` for one tenant-scoped posture projection combining controls, obligations, policy gaps, evidence-request workflow state, and versioned risk assessments; treatment plans and risk acceptances remain explicit gaps.
...
| Create and assess a risk | `POST /risk-methodologies`, `POST /risks`, `POST /risks/{id}/assessments`, or `GET /risks`; assessments require a versioned method and expected revision |
...
| Read the tenant-scoped compliance workspace | `GET /compliance-workspace` with `compliance_governance`; returns distinct obligations, work-item queues, controls, policy gaps, evidence-request state, and risk assessments |
