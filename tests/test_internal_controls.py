...
def test_internal_control_status_projection_covers_failure_exception_stale_and_not_applicable() -> None:
    ...
    with factory() as session:
        ...
        inactive = _foundation(session, decision, "IC-INACTIVE-1")
        _map(session, decision, inactive, "10.3.2")
        ...
        record_control_test_result(
            ...
            ControlTestResultCode.INEFFECTIVE.value,
            "The inactive plan result must not affect current coverage.",
        )
        inactive_plan.active = False
        session.flush()
        inactive_control = get_control_item(session, FrameworkCode.CSAP_2026, "10.3.2")
