...
flowchart LR
    ...
    kernel --> risks[(versioned risk methodologies, register, assessments, treatments, acceptances, and closures)]
...
| `risk_treatment_plan` | Immutable versioned risk response with strategy, owner, due date, and status |
| `risk_acceptance` | Immutable independent, time-bounded above-appetite acceptance with optional escalation reference |
| `risk_closure` | Immutable independent closure approval tied to the latest within-appetite assessment and evidence reference |
...
- `risk_assessment` pairs tenant, risk, and methodology identifiers; assessment links pair tenant, assessment, implemented control, test result, and evidence usage identifiers. Treatment, acceptance, and closure rows pair tenant and risk identifiers; acceptance and closure also pair the exact assessment being acted on.
