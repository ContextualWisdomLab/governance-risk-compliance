...
- Never treat `X-Actor-Id` or `X-Purpose` as authentication. When a Keyverse verifier is configured, the verified access-token subject is the actor. The HTTP surface remains loopback-only until tenant authorization and deployment hardening are implemented.
