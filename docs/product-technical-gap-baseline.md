...
| G-02 | P0 | Operators can still encounter ambiguous schema ownership and migration failure modes in a durable PostgreSQL deployment. | Schema lifecycle implementation is in PR #18; the current head `bc47c10` tightens PostgreSQL acceptance cleanup and trigger-timing assertions after the earlier `514e970`, `97aa541`, and `41dd68f` fixes. | Merge only after fresh exact-head Product, PostgreSQL, SAST, security, and independent review evidence; then document deploy, expand/backfill/contract, rollback, and retry order. | [Issue #8](https://github.com/ContextualWisdomLab/governance-risk-compliance/issues/8) |
...
| G-05 | P0 | Operators lack production readiness, SLO, telemetry, and incident evidence. | `develop` still exposes only the old constant `/healthz`; PR #22 adds dependency-separated `/readyz` and `/startupz`, startup admission checks, drain rejection, W3C correlation, safe JSON request logs; PR #23 adds W3C-parented OpenTelemetry request spans, request rate/duration, authorization-denial metrics, and route-template cardinality protection; PR #24 adds request-scoped database transaction and audit-write outcome/duration/count metrics; PR #25 adds a proposed 30-day SLO/error-budget and multi-window burn-rate policy; PR #26 adds bounded SQLAlchemy database-pool gauges; PR #31 adds a bounded declared-recovery event contract on the unmerged stacked heads. | Complete owner approval, collector acceptance, dashboards, paging, rollback/restore rehearsal, and production integration evidence. | [Issue #11](https://github.com/ContextualWisdomLab/governance-risk-compliance/issues/11) |
...
| --- | --- | --- | --- |
| [#18](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/18) | `bc47c10` | Ready, mergeable, blocked | Current head adds explicit PostgreSQL engine ownership in acceptance tests, guaranteed probe-table cleanup, and statement-time trigger assertions. Product/PostgreSQL/SAST/Security checks were newly queued; local full coverage and real PostgreSQL acceptance passed. No independent approval was observed; merge only after terminal success on `bc47c10` and approval. |
...
| [#22](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/22) | `47be550` | Ready, mergeable, review-required, blocked | Operational readiness slice stacked on PR #21. Exact-head Product Checks are green and local Python 3.12 coverage is 100%; no independent formal approval was observed. OpenTelemetry, SLO, alerting, and production remote-boundary work remain open. |
| [#23](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/23) | `fac1c00` | Ready, mergeable, review-required, blocked | Telemetry slice stacked on PR #22. Exact-head Product Checks are green; live PostgreSQL 18.6 readiness and telemetry probe passed; no independent formal approval was observed. Collector acceptance, database metrics, SLO, alerting, and production remote-boundary work remain open. |
| [#24](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/24) | `84c1265` | Ready, mergeable, review-required, blocked | Database telemetry slice stacked on PR #23. Exact-head Product Checks are green; live PostgreSQL 18.6 transaction and audit-write metrics probe passed; no independent formal approval was observed. Pool metrics, collector acceptance, SLO, alerting, and production remote-boundary work remain open. |
| [#25](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/25) | `a62d66e` | Ready, mergeable, review-required, blocked | Proposed SLO/error-budget policy stacked on PR #24. Exact-head Product Checks are green; owner approval, collector delivery, dashboards, paging, and recovery rehearsal remain unobserved. |
| [#26](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/26) | `7564e17` | Ready, mergeable, review-required | Bounded database-pool telemetry stacked on PR #25. Exact-head Product #442 is green; CodeRabbit did not review the non-default stacked base, and no independent formal approval was observed. Collector delivery, recording rules, dashboards, paging, recovery rehearsal, owner approval, and production integration remain open. |
| [#31](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/31) | `2e34c86` | Draft, checks pending | Declared recovery-event telemetry stacked on PR #26. Local 100% statement/branch coverage and live PostgreSQL pool/recovery probe passed; Product #450 is queued, CodeRabbit skipped the Draft, and no independent approval was observed. Recovery coordination, collector delivery, recording rules, dashboards, paging, restore/rollback rehearsal, owner approval, and production evidence remain open. |
