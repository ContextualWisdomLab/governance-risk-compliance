...
| G-05 | P0 | Operators lack production readiness, SLO, telemetry, and incident evidence. | `develop` still exposes only the old constant `/healthz`; PR #22 adds dependency-separated `/readyz` and `/startupz`, startup admission checks, drain rejection, W3C correlation, safe JSON request logs, and a reason-code runbook; PR #23 adds W3C-parented OpenTelemetry request spans, request rate/duration, authorization-denial metrics, and route-template cardinality protection on unmerged stacked heads. | Complete collector acceptance, database/pool/transaction metrics, SLOs, alert thresholds, dashboards, paging, rollback/restore rehearsal, and production integration evidence. | [Issue #11](https://github.com/ContextualWisdomLab/governance-risk-compliance/issues/11) |
