...
Develop-based buyer-workspace head: `eafa57c07cbbf405d4d60c914e2a76e557037437`
...
Chromium interaction tests. The latest hosted exact-head observation has 19
successful, 1 queued, and 8 skipped check runs with no failure conclusion; the
remaining queue is Required OpenCode Review `coverage-evidence` run
`32517277945`.
The remaining checks are terminal success or skipped in this observation.
Independent approval is absent. It is non-Draft; the fixture is not authentication, API,
...
Its current exact head `b3a6d1a09f28f257bf61600809abf511384498c6` adds the
...
79 passing tests with 100% statement/branch/docstring coverage, Ruff, compile,
lock, actionlint, diff, and Semgrep zero-blocking evidence. The current repair
also skips policy migration DDL on partial stores, enforces matching source
version/import-run release links at the SQLite/PostgreSQL boundary, and fails
closed on mismatched release snapshots. A real PostgreSQL 18.4 probe verifies
all three migration receipts, idempotent re-entry, and the version guard.
After rerunning the failed job, the current hosted observation is 19
successful, 1 in-progress, and 7 skipped checks. The prior `strix` run was
provider infrastructure evidence (NVIDIA NIM rate limit and GitHub Models
retirement brownout), not a source finding; no independent approval exists.
...
evidence, and the latest hosted observation is 18 success, 1 queued, and 8
skipped checks after the changelog repair; Required OpenCode Review
`coverage-evidence` remains queued; independent approval and last-push approval
remain absent.
...
current hosted observation is 15 successful, 1 failed, and 9 skipped. The
prior `osv-scan` check failed the central workflow
output-contract after scanner exit 0, not on a source-vulnerability verdict;
...
and legacy-upgrade rehearsals. Gitleaks and osv-scanner binaries are
...
| Preview posture | PR #54 stages `GET /workspace/posture` on the PR #34 buyer-workspace head, reusing the official catalog, legacy evidence binding, and policy-gap queries while exposing exact `not_assessed` and `unknown` rows. | The projection is local-only, not tenant-scoped, and deliberately reports zero effective controls until Keyverse authorization, internal controls, and effectiveness tests exist. |
...
| --- | --- | --- | --- | --- | --- |
| G-01 | P0 | A customer cannot safely use the product remotely because caller identity and tenant are not verified. | Local-only boundary in `cwl_grc/remote_access.py`; issue #4; closed PR #5 is superseded by direct-to-`develop` PR #38, while PRs #6, #7, and #16 are closed stack-only merges that did not update protected `develop`. | Verify issuer, audience, token type, signature, actor, tenant, workspace, client, principal kind, role, purpose, and action scope on every protected read/write/export/background job. | [Issue #4](https://github.com/ContextualWisdomLab/governance-risk-compliance/issues/4) |
...
| G-09 | P0 | A green workflow can be mistaken for release authority without one exact machine-verifiable readiness decision. | PR #17 exact head `ebf24a6ec69855a794dc03f4c665a958c37e17a3` adds a fail-closed readiness manifest, exact repository-file/index binding, and no-follow regular-file reads after repairing the exact-head Strix TOCTOU finding; it remains unmerged and intentionally reports `production_ready=false`. | Require exact current source/artifact evidence, freshness, branch policy, security gates, independent approval, and zero blockers before release mode succeeds. | [Issue #15](https://github.com/ContextualWisdomLab/governance-risk-compliance/issues/15) |
| G-10 | P1 | Catalog maintenance can drift, lose provenance, overstate mappings, or redistribute source text without authority. | Catalog rows are manually checked into `cwl_grc/catalog.py`; PR #37 exact head `b3a6d1a09f28f257bf61600809abf511384498c6` stages provenance metadata, source/version/import/receipt/release persistence, a governed release-review list, metadata-only release detail and published-only comparison, explicit content-policy mapping, reviewed APA references, a server-owned source-host boundary, matching source-version database guards, and a locked PostgreSQL driver, but no remote fetch, OSCAL/OLIR parser, requirement-level diff, or `control_framework` publication is claimed. | Add lawful source-artifact ingestion, requirement-level versioned release/diff, export restrictions, OSCAL 1.2.3 Catalog/Profile/Mapping round trips, reviewed mapping semantics, OLIR provenance, and change impact. | [Issue #29](https://github.com/ContextualWisdomLab/governance-risk-compliance/issues/29) |
| G-11 | P1 | A buyer lacks a role-aware compliance workspace, evidence request room, external-auditor data room, and controlled export. | PR #34 exact head `eafa57c07cbbf405d4d60c914e2a76e557037437` carries the Figma/Storybook authority, semantic design tokens, exact-value and next-action fixture states including the Open deficiencies row, lockfile/Node pinning, Chromium interaction evidence, rendered desktop-header/content and mobile layout repair, and English/Korean i18n contract. Stacked PR #54 exact head `bb18be1d5fbf8b61af524cb99829c405b98c1e72` adds a local-only posture projection that reuses the catalog, legacy evidence, and policy-gap queries, includes catalog edition/source provenance, and lists exact policy gaps without evidence payloads; historical PR #40 was merged into the buyer-workspace feature branch as `59e1a3cce5a794f1b6a33a9f5128a1ccccf95109`, and PR #45 added a tenant-scoped request/submission/review workflow on another feature branch. No protected merge is observed. | Deliver tenant- and purpose-scoped posture, traceability, real evidence requests, action queues, accessible exact-value views, reproducible CSV/JSON/PDF exports, i18n consistency, and expiring/revocable auditor packages. | [Issue #30](https://github.com/ContextualWisdomLab/governance-risk-compliance/issues/30) |
...
| [#17](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/17) | `ebf24a6ec69855a794dc03f4c665a958c37e17a3` (2026-08-22) | Ready, merge-blocked, review-required | Exact-head repair closes the readiness evidence TOCTOU finding with no-follow descriptor reads and refreshed index bindings. Local evidence is 115 passing tests, 1,021 statements/256 branches at 100%, Interrogate 100%, Ruff, compile, lock, actionlint, production-readiness validation, wheel/install smoke, Semgrep 0, and pip-audit clean. Hosted runs were recreated after the push (2 success, 13 pending, 7 skipped at observation); no independent approval or last-push approval exists. |
...
| [#34](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/34) | `eafa57c07cbbf405d4d60c914e2a76e557037437` (2026-08-22) | Ready, merge-blocked, review-required | Issue #30 buyer-workspace design authority on `develop`; the exact head adds the Open deficiencies exact-value row, access-denied Request access action, explicit local-developer-preview boundary in both locales, CSS lint repair, release-specific NIST SP 800-53 Release 5.2.0 sources, Storybook reuse of the real index markup, and the checked-in Playwright binary workflow command. Storybook build, five-story Chromium checks, three page interactions, and six Python design-contract tests pass; latest hosted observation is 19 success, 1 queued, and 8 skipped, with Required OpenCode Review `coverage-evidence` queued and independent approval absent. |
| [#37](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/37) | `b3a6d1a09f28f257bf61600809abf511384498c6` (2026-08-22) | Ready, merge-blocked, review-required | Issue #29 catalog-provenance slice on `develop`; the current repair accepts explicit HTTPS `:443` while rejecting non-default ports, skips policy DDL on partial stores, and enforces matching release/import source versions at database and snapshot boundaries. Exact-head local evidence is 79 passing tests with 100% statement/branch/docstring coverage, PostgreSQL clean-install/idempotent migration evidence, and quality/security checks green. After rerunning the failed job, current hosted observation is 19 success, 1 in-progress, and 7 skipped; the prior Strix failure was provider infrastructure, and independent approval is absent. |
| [#38](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/38) | `93ca6383d2b14c6793ecdfe42c5da1f400168a35` (2026-08-22) | Ready, merge-blocked, review-required | Replacement for closed PR #5. The exact head adds the optional caller-owned atomic JTI replay guard, scope-before-consume ordering, undefined-offset provider-clock rejection, and the ADR 0004 changelog entry; local evidence is 98 passing tests with 100% statement/branch/docstring coverage plus lint/compile/lock/actionlint/Semgrep/pip-audit, while hosted observation is 18 success, 1 queued, and 8 skipped; Required OpenCode Review `coverage-evidence` is queued, and independent approval is absent. |
