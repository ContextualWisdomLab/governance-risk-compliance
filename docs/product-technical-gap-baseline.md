...
Its current exact head `9211ac3d6ce83da2b26f0283bf893dcfa6ddd294` adds the
governed bounded published-release review list, metadata-only release detail
snapshot, and published-only metadata comparison after the server-owned
source-host boundary; the locked runtime includes `psycopg[binary]`, the local
suite has 70 passing tests with 100% statement/branch/docstring coverage, and a
real PostgreSQL 18.6 probe verifies migration `0002_catalog_provenance`, a 200
...
| [#37](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/37) | `9211ac3d6ce83da2b26f0283bf893dcfa6ddd294` (2026-08-21) | Draft, mergeable, review-required | Issue #29 catalog-provenance slice on `develop`. Current head adds bounded published-only release listing, governed metadata-only detail, and published-only metadata comparison; the locked runtime includes `psycopg[binary]`, local 70-test 100% statement/branch/docstring, Ruff, compile, lock, actionlint, diff, and Semgrep zero-blocking evidence pass, plus a real PostgreSQL 18.6 migration and exact-head published/withdrawn comparison probe. Product is successful while CodeQL, SAST, Security, Strix, and required review workflows remain pending; scope excludes remote source retrieval, OSCAL/OLIR parsing, requirement-level control diff, and two approving reviews plus last-push approval remain outstanding. |
