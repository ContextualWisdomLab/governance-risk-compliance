...
`develop`. Its exact head is `2e02fa266a731d3c2079770a486612a86a952d6a` on
...
| [#53](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/53) | `2e02fa266a731d3c2079770a486612a86a952d6a` (2026-08-22) | Open, normal-merge candidate, review/checks pending | First G-06 version-one policy API contract on `develop`: strict models, bounded cursor pagination, durable idempotency, ETags/If-Match, RFC 9457 errors, OpenAPI deprecation markers, deterministic mapping serialization for stable ETags, and explicit loopback-only preview boundary. Exact-head local evidence passes 100% statement/branch/docstring coverage, Ruff, compile, lock, actionlint, wheel/install smoke, Semgrep zero findings, and pip-audit no known vulnerabilities. Hosted required checks and independent review must be re-queried before merge; Gitleaks/osv-scanner were unavailable locally. |
