...
| G-03 | P0 | Loss, rotation, or legal hold of an evidence key can make operational evidence unavailable or non-compliant. | PR #20 adds versioned key metadata, explicit old/new key overlap, context and integrity checks, legacy migration, and audited bounded rewrap; it remains Draft and unmerged. | Add KMS/HSM integration, persistent re-encryption job receipts, recovery rehearsal evidence, retention disposition, legal hold, and purpose-specific export without destructively masking authorized PII. | [Issue #9](https://github.com/ContextualWisdomLab/governance-risk-compliance/issues/9) |
