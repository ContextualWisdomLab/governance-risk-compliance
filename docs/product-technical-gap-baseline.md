...
The next production action is to attribute Keyverse issuer, client, and
correlation on `audit_event` without copying the raw token, then obtain
independent approval of the Keyverse stack. Remote exposure, exports, and
background jobs remain closed. The next product-model action is issue #27:
...
| --- | --- | --- | --- | --- | --- |
| G-01 | P0 | An officer cannot safely use the product remotely because caller identity and tenant are not verified. | Local-only boundary in `cwl_grc/remote_access.py`; issue #4; PR #38 `c1445d7105ee7366794a0acb0d0507777850ce9e` verifies RFC 9068 access tokens and loads OIDC/JWKS; stacked PR #55 `5990347f4f18d5f5ee6106b383a4729945a1cbbe` enforces Bearer tokens on policy/evidence routes, rejects actor-header impersonation, persists Keyverse `org` on owned rows (`local_preview` for legacy stores), isolates reads/writes by tenant, and publishes `KeyverseBearer` (`at+jwt`) plus policy/evidence scopes on `/openapi.json`. Local 106-test 100% statement/branch/docstring evidence. Remote traffic remains denied. | Complete Keyverse audit-event issuer/client/correlation attribution, deployment-hardening runbooks, and remote exposure only after identity and tenant authorization are independently approved. | [Issue #4](https://github.com/ContextualWisdomLab/governance-risk-compliance/issues/4) |
...
| [#55](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/55) | `5990347f4f18d5f5ee6106b383a4729945a1cbbe` (2026-08-23) | Ready, stacked on #38 | Keyverse HTTP adapter plus tenant-owned persistence and OpenAPI: Bearer RFC 9068 access tokens on policy/evidence routes and officer home, actor-header impersonation rejected, tenant header must match `org`, owned rows stamp `tenant_identifier`, officer-scoped policy list/gaps/home, `/openapi.json` publishes `KeyverseBearer`. Local 106-test 100% statement/branch/docstring evidence. Must stay behind PR #38. Independent review remains the merge blocker. |
...
keep PR #55 stacked on PR #38 at `5990347f4f18d5f5ee6106b383a4729945a1cbbe`;
merge ContextualWisdomLab/.github#1257; implement Keyverse audit-event
issuer/client/correlation attribution as the next G-01 slice without enabling
remote exposure.
