...
`32517277945`.
Analyze, Semgrep, dependency review, OSV,
...
After rerunning the failed job, the current hosted observation is 18
successful, 2 queued, and 7 skipped checks. The prior `strix` run was provider
infrastructure evidence (NVIDIA NIM rate limit and GitHub Models retirement
brownout), not a source finding; no independent approval exists.
...
evidence, and the latest hosted observation is 16 success, 1 in-progress, 1
queued, and 7 skipped checks after the changelog repair; Strix is in progress
and Required OpenCode Review `coverage-source-tree` remains queued; independent
approval and last-push approval remain absent.
...
after rerunning the failed job, current hosted observation is 15 successful, 1
queued, and 9 skipped. The prior `osv-scan` check failed the central workflow
...
| G-11 | P1 | A buyer lacks a role-aware compliance workspace, evidence request room, external-auditor data room, and controlled export. | PR #34 exact head `e9d25798f53d7ac13c1a6cdf02c54c16fa82b150` carries the Figma/Storybook authority, semantic design tokens, exact-value and next-action fixture states including the Open deficiencies row, lockfile/Node pinning, Chromium interaction evidence, rendered desktop-header/content and mobile layout repair, and English/Korean i18n contract; the stack-only PR #40 was merged into the prior buyer-workspace feature head as `59e1a3cce5a794f1b6a33a9f5128a1ccccf95109`, and historical PR #45 added a tenant-scoped request/submission/review workflow on another feature branch. The UI remains a static local preview and no protected merge is observed. | Deliver tenant- and purpose-scoped posture, traceability, real evidence requests, action queues, accessible exact-value views, reproducible CSV/JSON/PDF exports, i18n consistency, and expiring/revocable auditor packages. | [Issue #30](https://github.com/ContextualWisdomLab/governance-risk-compliance/issues/30) |
...
| [#37](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/37) | `b3a6d1a09f28f257bf61600809abf511384498c6` (2026-08-22) | Ready, merge-blocked, review-required | Issue #29 catalog-provenance slice on `develop`; the current repair accepts explicit HTTPS `:443` while rejecting non-default ports, skips policy DDL on partial stores, and enforces matching release/import source versions at database and snapshot boundaries. Exact-head local evidence is 79 passing tests with 100% statement/branch/docstring coverage, PostgreSQL clean-install/idempotent migration evidence, and quality/security checks green. After rerunning the failed job, current hosted observation is 18 success, 2 queued, and 7 skipped; the prior Strix failure was provider infrastructure, and independent approval is absent. |
| [#38](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/38) | `93ca6383d2b14c6793ecdfe42c5da1f400168a35` (2026-08-22) | Ready, merge-blocked, review-required | Replacement for closed PR #5. The exact head adds the optional caller-owned atomic JTI replay guard, scope-before-consume ordering, undefined-offset provider-clock rejection, and the ADR 0004 changelog entry; local evidence is 98 passing tests with 100% statement/branch/docstring coverage plus lint/compile/lock/actionlint/Semgrep/pip-audit, while hosted observation is 16 success, 1 in-progress, 1 queued, and 7 skipped; Strix is in progress, Required OpenCode Review `coverage-source-tree` is queued, and independent approval is absent. |
