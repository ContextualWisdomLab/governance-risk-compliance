...
Tracked current open direct-to-develop heads: PR #17 `7b4adbe0964e12e4b31e197ab10a63aabf0f593c`, PR #18 `35d3e55ccd2c5e89efec6c28b7613f1d605cac13`, PR #34 `ed1e6222df0ff021fb72d40d79a242bae71754c4`, PR #37 `9fffa505512d4cf7eccbb6cd240dab6a1e8acb73`, PR #38 `c1445d7105ee7366794a0acb0d0507777850ce9e`, PR #43 `f41c387c62998de2d136ce8f4a110b4cb22906ac`, PR #51 `1a8f90dd15f37ffc86b8a0efd217a8b2812e5f99`, and PR #53 `976945dbbe22b0b8fa7893150e2723738a0ff484`. PR #19 is the current document branch; stacked PR #54 is `3e972f4de9e9a1f5a17379ac93b33bdcb2b0f4cf` on current PR #34 head `ed1e6222df0ff021fb72d40d79a242bae71754c4`; stacked PR #55 is `531121b44424b6b7d68d84f722c6e91eaffd4c2b` on PR #38 `c1445d7105ee7366794a0acb0d0507777850ce9e`; stacked PR #56 is `86795a2c2440842c6d5bd18020fb5af87470a995` on PR #55; stacked PR #57 is `ca85f6852658d8ec3b10f2241be8e26c6edcd258` on PR #56; PR #41 is feature-base PR head `2346afe6a063f502517ee235d6c3e87488da8357` on base `8881d3966e3e9da29b8dce990a95295a2780618b`; re-query all exact heads before merge.
...
The next production action is to obtain independent approval of the Keyverse
stack (PRs #38, #55, #56, #57). Remote exposure, exports, and background jobs
remain closed. `CWL_GRC_REQUIRE_KEYVERSE` is a loopback TLS start, not customer
admission. The next product-model action is issue #27:
establish internal control, implementation, testing, effectiveness, deficiency,
and evidence-usage truth before risk and audit workflows depend on the current
catalog model.
...
| --- | --- | --- |
| Policy authoring | `policy_document`, immutable `policy_version`, and `policy_control_mapping`; HTTP and CLI paths exist. PR #55 stamps Keyverse `org` on policy rows and isolates officer reads/writes by tenant. | No approval/publication workflow, effective period, or scheduled review. Production identity still requires protected merge of the Keyverse stack. |
...
| Evidence | Evidence is encrypted at rest, exact operational values remain usable, and artifacts bind to catalog rows. PR #55 stamps Keyverse `org` on evidence and audit rows and fails closed on cross-tenant bind. | Rotation, KMS/HSM, recovery rehearsal, disposition, request/review workflow, and purpose-specific exports remain incomplete. |
...
| --- | --- | --- | --- | --- | --- |
| G-01 | P0 | An officer cannot safely use the product remotely because caller identity and tenant are not verified. | Local-only boundary in `cwl_grc/remote_access.py`; issue #4; PR #38 `c1445d7105ee7366794a0acb0d0507777850ce9e` verifies RFC 9068 access tokens and loads OIDC/JWKS; stacked PR #55 `531121b44424b6b7d68d84f722c6e91eaffd4c2b` enforces Bearer tokens, tenant isolation, and OpenAPI `KeyverseBearer`; stacked PR #56 `86795a2c2440842c6d5bd18020fb5af87470a995` attributes `audit_event` issuer/client/correlation without copying the token (117 tests); stacked PR #57 `ca85f6852658d8ec3b10f2241be8e26c6edcd258` requires an injected verifier and loopback TLS when `CWL_GRC_REQUIRE_KEYVERSE` is set (121 tests, 100% coverage). Remote traffic remains denied. | Independent approval of the Keyverse stack. Remote customer admission remains closed. | [Issue #4](https://github.com/ContextualWisdomLab/governance-risk-compliance/issues/4) |
...
| [#34](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/34) | `ed1e6222df0ff021fb72d40d79a242bae71754c4` (2026-08-23) | Ready, merge-blocked, review-required | Issue #30 officer-workspace design authority on `develop`; the exact head adds Storybook CSF `play` functions for the ten UX dimensions, officer/organization language instead of customer-facing “Buyer” copy, Figma file `ta1jjWSjmADz2BFxka9UPs`, design tokens, Open deficiencies exact-value row, and Chromium i18n evidence. Local 53-test Python suite, 100% statement/branch/docstring coverage, Storybook 10.5.10 build, and four Playwright checks pass. Independent approval and last-push approval are absent; hosted checks on this new head were not yet terminal at snapshot time. |
...
| [#55](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/55) | `531121b44424b6b7d68d84f722c6e91eaffd4c2b` (2026-08-23) | Ready, stacked on #38 | Keyverse HTTP adapter plus tenant-owned persistence, OpenAPI `KeyverseBearer`, officer-home Bearer forms, and typed insufficient-scope 403. Local evidence on later stacked heads. Must stay behind PR #38. Independent review remains the merge blocker. |
| [#56](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/56) | `86795a2c2440842c6d5bd18020fb5af87470a995` (2026-08-23) | Ready, stacked on #55 | Keyverse audit attribution: issuer, OAuth client, `X-Request-ID` correlation, and `allow` on `audit_event` without copying the token; migration `0003_audit_attribution`; local-preview hardening runbook. 117 tests, 100% coverage. Independent review remains the merge blocker. |
| [#57](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/57) | `ca85f6852658d8ec3b10f2241be8e26c6edcd258` (2026-08-23) | Ready, stacked on #56 | Hardened local start: `CWL_GRC_REQUIRE_KEYVERSE` refuses header-identity boots and admits only loopback TLS. HTTP `/healthz` returns 503 under that flag. Unset preview is unchanged. 121 tests, 100% coverage. Not remote production exposure. Independent review remains the merge blocker. |
...
keep PR #55 stacked on PR #38 at `531121b44424b6b7d68d84f722c6e91eaffd4c2b`;
keep PR #56 stacked on PR #55 at `86795a2c2440842c6d5bd18020fb5af87470a995`;
keep PR #57 stacked on PR #56 at `ca85f6852658d8ec3b10f2241be8e26c6edcd258`;
merge ContextualWisdomLab/.github#1257. Do not enable remote production
exposure. The next G-01 remainder is non-loopback customer admission after
independent review.
