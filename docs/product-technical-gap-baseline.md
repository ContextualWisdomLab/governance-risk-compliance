...
that central PR remains open, merge-blocked, with OpenCode `CHANGES_REQUESTED`
because exact-head Strix is `FAILURE`. This fork head's hosted observation is
...
open until its exact-head Strix failure and OpenCode `CHANGES_REQUESTED` are
resolved in the central control plane; do not enable non-loopback customer
