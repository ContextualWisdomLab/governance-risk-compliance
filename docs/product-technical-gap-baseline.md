...
independent approval remains absent. It remains unmerged and depends on PR
#32's protected independent-review gate.
...
Its current exact head `b685cd739efa1f5d285b821676e841b6a629186d` adds the
governed release-review list after the server-owned source-host boundary; the
local suite has 69 passing tests with 100% statement/branch/docstring coverage,
and a real PostgreSQL 18.6 probe verifies migration `0002_catalog_provenance`
and `GET /catalog/releases`. Product #616 is terminal success and SAST/Security
#205 are queued. It deliberately does not claim remote fetching, OSCAL/OLIR
parsing, or `control_framework` publication; independent formal approval
remains outstanding.
...
| [#33](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/33) | `7922d9be1aca16a0ffbd59d44e0183dd2ef7155b` (2026-08-21) | Ready, stacked, review-required | Obligation applicability and regulatory-change model stacked on current PR #32 for Issue #28. Current head has 166 passing tests with 100% statement/branch/docstring coverage, Ruff, compile, lock, actionlint, Semgrep zero blocking findings, and a real PostgreSQL 18.6 migration/index/trigger/tenant/immutability probe. Product #608 is terminal success and SAST/Security #201 are queued; independent approval remains outstanding. |
...
| [#37](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/37) | `b685cd739efa1f5d285b821676e841b6a629186d` (2026-08-21) | Draft, mergeable, review-required | Issue #29 catalog-provenance slice on `develop`. Current head adds the governed `GET /catalog/releases` review list; local 69-test 100% statement/branch/docstring, Ruff, compile, lock, actionlint, diff, and Semgrep zero-blocking evidence pass, plus a PostgreSQL 18.6 migration and endpoint probe. Product #616 is terminal success and SAST/Security #205 are queued; scope excludes remote source retrieval and OSCAL/OLIR parsing, and independent approval remains outstanding. |
