...
| G-03 | P0 | Loss, rotation, or legal hold of an evidence key can make operational evidence unavailable or non-compliant. | Durable Fernet material is required, but no rotation/recovery/retention workflow is in the first slice. | Add versioned key-encryption metadata, dual-read rotation, explicit re-encryption ownership, recovery rehearsal evidence, retention deletion, legal hold, and audit records without destructively masking authorized PII. | [Issue #9](https://github.com/ContextualWisdomLab/governance-risk-compliance/issues/9) |
...
| [#16](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/16) | `cf69534` | Draft, mergeable | Product checks observed green. Keep stacked behind the verified JWT, OIDC, and route-enforcement sequence; do not mark ready without exact-head tenant isolation evidence. |
| [#7](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/7) | `a5c6f14` | Draft, mergeable | Product checks observed green. Review route enforcement against the verified principal and scope contract before advancing. |
| [#6](https://github.com/ContextualWisdomLab/governance-risk-compliance/pull/6) | `3a466a6` | Draft, mergeable | Product checks observed green. Keep discovery/JWKS loading bounded and pinned; do not expose remote traffic by itself. |
