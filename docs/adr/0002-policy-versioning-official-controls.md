...
> Superseded for coverage semantics by ADR 0011. The historical first-slice
> binding remains supported as compatibility data, but it is `unassessed` and
...
A compliance officer therefore needs to author a policy, keep editions, and see which mapped requirements still lack an operating-effective conclusion. The first slice already owned one official control catalog. A second catalog or a policy-engine dialect would invent a competing truth.
...
3. Treat a policy gap as a latest-edition mapping whose internal-control status is not operating-effective or authorized not-applicable. Keep direct `control_evidence_binding` rows as compatibility data while internal-control tests establish effectiveness.
...
Officers can write a policy, see an uncovered mapped control status, and establish the next control test without leaving CWL GRC. Residual risk scoring and audit-workflow bodies remain later work. Peer CWL services still consume control/evidence contracts only.
