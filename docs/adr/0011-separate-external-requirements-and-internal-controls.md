...
- API and UI work must expose `unknown`, `unassessed`, `implemented_not_tested`,
  ...
  `stale`, and authorized `not_applicable` accurately. These are projections
  over separate implementation, effectiveness, freshness, exception, and
  applicability facts; the projection records source identifiers, precedence,
  as-of time, and recalculation history rather than replacing those facts.
